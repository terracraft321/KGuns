...
void onRender(CSprite@ this) {
	...
	const SColor YELLOW = SColor(255, 150, 255, 0);
	...
	Vec2f center = b.getPosition();
	const f32 renderRadius = (b.getRadius()) * 0.95f;		 
		Vec2f pos2d = b.getScreenPos();
		Vec2f dim = Vec2f(24,8);
		const f32 y = b.getHeight()*2.4f;
		if (b.get_u8("chargeTimer") > 0 && SNIPER)
		{
			const f32 perc = b.get_u8("chargeTimer") / SNIPER_TIME;
			if (perc >= 0.0f)
			{
				GUI::DrawRectangle( Vec2f(pos2d.x - dim.x-2, pos2d.y + y-2), Vec2f(pos2d.x +dim.x+2, pos2d.y + y + dim.y+2) );
				GUI::DrawRectangle( Vec2f(pos2d.x - dim.x+2, pos2d.y + y+2), Vec2f(pos2d.x - dim.x + perc*1.0f*dim.x -2, pos2d.y + y + dim.y-2), YELLOW );
			}
		}
}